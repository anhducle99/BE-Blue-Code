  ...
  try {
    ...
    if (userRole === "SuperAdmin") {
      ...
      if (isNaN(parsed) || parsed <= 0) {
        return res
          .status(400)
          .json({ success: false, message: "SuperAdmin cần truyền organization_id" });
      }
    ...
    const list = await IncidentCaseModel.findByOrganization(organizationId, {
      startDate,
      endDate,
    });
    const allHandlerKeys = list.flatMap((row) => row.handlerKeys);
    const statusMap = await HandlerStateModel.getStatusMapForHandlers(
      allHandlerKeys,
      organizationId
    );
      ...
      const handlers = row.handlerKeys.map((key) => {
        ...
        const status =
          currentId == null
            ? "available"
            : currentId === row.id
              ? "handling_this_incident"
              : "handling_other_incident";
  ...
  try {
    ...
    if (!dbUser || !dbUser.name) {
      return res.status(403).json({
        success: false,
        message: "Không xác định được handler từ tài khoản đăng nhập",
      });
    }
    ...
    try {
      if (caseRow.organizationId !== dbUser.organization_id) {
        return res.status(403).json({
          success: false,
          message: "Không có quyền thao tác với sự cố này",
        });
      }
...
export const releaseIncident = async (req: Request, res: Response) => {
  try {
    ...
    const handlerKey = String(dbUser.name).trim();
    const incidentCaseId = await HandlerStateModel.getCurrentIncidentCaseId(
      handlerKey,
