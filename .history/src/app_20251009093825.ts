...
io.on("connection", (socket) => {
  ...
  socket.on("register", (phone) => {
    console.log(
      "📱 register called from socket:",
      socket.id,
      "identifier:",
      phone
    );
    onlineUsers[String(phone)] = socket.id;
  ...
  socket.on("disconnect", () => {
    ...
    // cleanup
    for (const p in onlineUsers) {
      if (onlineUsers[p] === socket.id) {
        delete onlineUsers[p];
        console.log("🗑 removed onlineUsers entry for", p);
      }
    }
    console.log("📍 onlineUsers now:", onlineUsers);
  });
