...
io.on("connection", (socket) => {
  ...
  socket.on("callAccepted", ({ callId, from, callerKey }) => {
    const caller = onlineUsers.get(callerKey);
    if (caller) {
      io.to(caller.socketId).emit("callAccepted", { callId, from });
    }
  });

  socket.on("callRejected", ({ callId, from, callerKey }) => {
    const caller = onlineUsers.get(callerKey);
    if (caller) {
      io.to(caller.socketId).emit("callRejected", { callId, from });
    }
  });
